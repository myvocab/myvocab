...
{exit;}
		...
		if (isset($_REQUEST['unlink']))
		{
			...
			if (file_exists($unlink) and $deleted == false)
			{
				...
				{
					unlink ($unlink);
					print "<center>Удаление файла $fl успешно завершено!</center><br>";
				}
       ...
       $get_full_name = "$dir/".$get_name;
       
       
       


$FullFileName = $get_full_name;
$FlSize = FileSize($FullFileName);
$str = "Content-Disposition: attachment; filename=".$get_name;
...
//				print " кб</td><td><a href=\"?do=download&dwld=$dir/$file\">[скачать]</a> <a href=\"javascript:del_wind('?do=download&unlink=$dir/$file', '$file')\">[удалить]</a>";
     			        print " кб</td><td><a href=111.php?do=download&dwld=$dir/$fl&df=$fl>[скачать]</a> <a href=\"javascript:del_wind('?do=download&unlink=$dir/$file&fl=$file', '$file')\">[удалить]</a>";
