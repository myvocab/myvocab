...
$wtr = mysqli_real_escape_string($link,$wtr);
$wprv = mysqli_real_escape_string($link,$wprv);
...
//$strSQL = 'Delete mvdone'.$userId.'.* FROM mvdone'.$userId.' WHERE (((mveditl'.$userId.'.wordO)="'.$wo.'"))' ;
//$result = mysqli_query($link, $strSQL); 
if ($we==$wprv){
